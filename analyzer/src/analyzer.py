...
def predict_score(team, opponent):
    ...
    tot_poss = (
        season_stat.tot_poss / team.count + previous_stat.tot_poss / team.previous_size
    ) / 2
...
class Analyzer:
    ...
    def run(self):
        ...
        for game in self.games:
            ...
            enough_previous_stats = (
                len(away_team.previous_stats) == self.previous_length
                and len(home_team.previous_stats) == self.previous_length
            )
    ...
    def count(self, total, actual, prediction, counter):
        if abs(total - prediction) > self.diff:
            if prediction < total:
                counter.win() if actual < total else counter.lose()
            else:
                counter.win() if actual > total else counter.lose()
        else:
            counter.skip()
